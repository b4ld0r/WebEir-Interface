			...
			.widget(
					"ui.multiselect",
					{
						...
						_create : function() {
							...
							this.availableActions = $(
									'<div class="actions ui-widget-header ui-helper-clearfix"><span class="busy">'
											...
											+ '</span><input type="text" class="search ui-widget-content ui-corner-all"/><a href="#" class="add-all">'
											+ $
							...
							this.availableActions.find('input').width(
									Math.max(this.availableActions.width()
											- this.availableActions.find(
													'a.add-all').width() - 30,
											20));
							// fix list height to match <option> depending on
							// their individual header's heights
						...
						_prepareLists : function(side, otherSide, opts) {
							...
							list
									.data('multiselect.sortable',
											opts[side].sortable)
									.data('multiselect.droppable',
											opts[side].droppable)
									.data(
											'multiselect.draggable',
											!opts[side].sortable
													&& (opts[otherSide].sortable || opts[otherSide].droppable) );
...
list.sortable({
											...
											receive : function(event, ui) {
												// DEBUG
												// that._messages(0, "Receive :
												// " +
												// ui.item.data('multiselect.optionLink')
												// + ":" +
												// ui.item.parent()[0].className
												// + " = " + itemSelected);

												// we received an element from a
												// sortable to another
												// sortable...
												if (opts[otherSide].sortable) {
													...
													// the other is droppable
													// only, so merely select
													// the element...
													setTimeout(function() {
														that._setSelected(
																ui.item,
							...
							// cannot be droppable if both lists are sortable,
							// it breaks the receive function
...
list.droppable({
											...
											drop : function(event, ui) {
												// DEBUG
												// that._messages(0, "drop " +
												// side + " = " +
												// ui.draggable.data('multiselect.optionLink')
												// + ":" +
												// ui.draggable.parent()[0].className);

												// alert( "drop " + itemSelected
												// );
												// if no optionLink is defined,
												// it was dragged in
												if (!ui.draggable
														.data('multiselect.optionLink')) {
													var optionLink = ui.helper
															.data('multiselect.optionLink');
													ui.draggable
															.data(
													...
													// received an item from a
													// sortable to a droppable
						...
						_registerAddEvents : function(elements) {
							...
							if (this.availableList
								...
								// refresh the selected list or the draggable
								// will not connect to it first hand
